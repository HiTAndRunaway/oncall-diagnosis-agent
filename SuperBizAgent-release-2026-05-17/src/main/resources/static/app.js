        ...
        try {
            ...
            if (data.code === 200 || data.message === 'success') {
                ...
                if (chatResponse && chatResponse.success) {
                    ...
                    // 读取后端返回的 sessionId 并存储
                    if (chatResponse.sessionId) {
                        this.sessionId = chatResponse.sessionId;
                        console.log('[sendQuickMessage] 更新 sessionId:', this.sessionId);
                    }
                                ...
                                if (sseMessage && typeof sseMessage.type === 'string') {
                                    if (sseMessage.type === 'content') {
                                        ...
                                        if (sseMessage.sessionId) {
                                            this.sessionId = sseMessage.sessionId;
                                            console.log('[SSE调试] 更新 sessionId:', this.sessionId);
                                        }
                                        this.handleStreamComplete(assistantMessageElement, fullResponse);
                                        return;
